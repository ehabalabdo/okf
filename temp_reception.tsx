...
const ReceptionView: React.FC<ReceptionViewProps> = ({ user: propUser }) => {
  ...
  // Duplicate invoice polling removed GÇö loadData already fetches invoices every 30s
  ...
  useEffect(() => {
    ...
    // +¬+¡+à+è+ä +º+ä+à+¦+¦+ë +à+å +º+ä+¦+è+¦+ü+¦ +à+¦ real-time subscription
    useEffect(() => {
        if (!user) return;
        
        console.log('[ReceptionView] =ƒö¦ Setting up subscription...');
        
        const unsubscribe = PatientService.subscribe(user, (data) => {
            // +º+ä+¬+¡+é+é +à+å +à+¦+è+¦ +¼+»+è+» +ä+ä+¦+ê+¬
            ...
            // +¡+ü++ +º+ä+à+¦+¦+ë +à+¿+º+¦+¦+¬ - +º+ä+ü+ä+¬+¦+¬ +¦+¬+¡+¦+ä +ü+è activeQueue
            setPatients(data);
        });
        
        return () => {
            console.log('[ReceptionView] =ƒö¦ Cleaning up subscription');
            unsubscribe();
        };
    }, [user]);
    ...
    const sendWhatsAppCredentials = (phone: string, name: string, password: string) => {
        ...
        const clinicName = client?.name || '+º+ä+¦+è+º+»+¬';
        
        // Prepare message in Arabic
        const message = [
          `+à+¦+¡+¿+º ${name}`,
          '',
          `+¬+à +¬+¦+¼+è+ä+â +ü+è +å+++º+à ${clinicName}`,
          '',
          '+¿+è+º+å+º+¬ +º+ä+»+«+ê+ä:',
          `+º+¦+à +º+ä+à+¦+¬+«+»+à: ${phone}`,
          `+â+ä+à+¬ +º+ä+à+¦+ê+¦: ${password}`,
          '',
          '+¦+º+¿++ +º+ä+»+«+ê+ä:',
          loginUrl,
          '',
          '+º+¡+¬+ü++ +¿+ç+¦+ç +º+ä+à+¦+ä+ê+à+º+¬ +¿+¦+â+ä +ó+à+å'
        ].join('\n');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await PatientService.add(user, {
                ...
                username: formData.phone, // +¦+é+à +º+ä+ç+º+¬+ü +ç+ê username
                email: undefined,
            ...
            if (formData.sendWhatsApp && patientPhone) {
                ...
                alert('G£à +¬+à+¬ +Ñ+¦+º+ü+¬ +º+ä+à+¦+è+¦ +¿+å+¼+º+¡!');
            }
        } catch (e: any) {
            alert("Error: " + (e.message || '+ü+¦+ä +Ñ+¦+º+ü+¬ +º+ä+à+¦+è+¦'));
        }
    };
  ...
  const handlePrintInvoice = async () => {
      ...
      // Use client (clinic) data GÇö each clinic gets their own name/logo/address
      const clinicName = client?.name || '+º+ä+¦+è+º+»+¬';
      ...
      const itemsHtml = selectedInvoice.items.map(item => `
        <tr>
          ...
          <td style="padding:12px 16px;border-bottom:1px solid #f1f5f9;font-size:14px;font-weight:700;color:#334155;text-align:left;white-space:nowrap">${item.price.toFixed(2)} +».+ú</td>
        </tr>
      `).join('');
...
<head>
  <meta charset="UTF-8"/>
  <title>+ü+º+¬+ê+¦+¬ - ${selectedInvoice.id.slice(-8)}</title>
  <style>
...
<body>
  <div class="header">
    ...
    <div>
      <div class="invoice-title">+ü+º+¬+ê+¦+¬</div>
  ...
  <div class="info-row">
    <div>
      <div class="info-label">+º+¦+à +º+ä+à+¦+è+¦</div>
      <div class="info-value">${selectedInvoice.patientName}</div>
    </div>
    <div class="info-right">
      <div class="info-label">+º+ä+¬+º+¦+è+«</div>
      <div class="info-value">${invoiceDate}</div>
    </div>
  </div>

  <table>
    <thead><tr><th>+º+ä+ê+¦+ü</th><th>+º+ä+à+¿+ä+¦</th></tr></thead>
    <tbody>${itemsHtml}</tbody>
  </table>

  <div class="total-bar">
    <div class="total-label">+º+ä+Ñ+¼+à+º+ä+è</div>
    <div class="total-amount">${selectedInvoice.totalAmount.toFixed(2)} +».+ú</div>
  </div>

  <div class="footer">+¦+â+¦+º+ï +ä+º+«+¬+è+º+¦+â+à ${clinicName}</div>
  ...
  <script>
    window.onload = function() {
      ...
      if (printWindow) {
          ...
          alert('+è+¦+¼+ë +º+ä+¦+à+º+¡ +¿+º+ä+å+ê+º+ü+¦ +º+ä+à+å+¿+½+é+¬ +ä+++¿+º+¦+¬ +º+ä+ü+º+¬+ê+¦+¬');
      }
  };
  ...
  // +º+ä+¦+â+¦+¬+è+¦+¬ +¬+¦+ê+ü +ü+é++ +º+ä+à+¦+¦+ë +º+ä+à+å+¬+++¦+è+å (+ä+à +è+¿+»+ú +º+ä+»+â+¬+ê+¦ +à+¦+ç+à +¿+¦+»)
  const activeQueue = React.useMemo(() => {
    const filtered = patients.filter(p => 
      p.currentVisit.visitId && 
      p.currentVisit.visitId.trim() !== '' && 
      p.currentVisit.status === 'waiting'  // +ü+é++ +º+ä+à+å+¬+++¦+è+å
    );
                                    ...
                                    <div className="bg-slate-50 p-4 rounded-xl text-left space-y-2 mb-4">
                                        {selectedInvoice.items.map((item, idx) => (
                                            <div key={idx} className="flex justify-between text-sm text-slate-600 border-b border-slate-200 pb-1 last:border-0 last:pb-0">
                                                ...
                                                <span className="font-mono font-bold">{item.price} +».+ú</span>
                                            </div>
                                        ))}
                                    </div>
                                    <div className="text-sm text-slate-400 uppercase">Total Due</div>
                                    <div className="text-4xl font-bold text-slate-800">{selectedInvoice.totalAmount} +».+ú</div>
                                </div>
                                ...
                                {invoices.length === 0 ? <div className="text-center py-10 text-slate-400">No pending invoices.</div> : invoices.map(inv => (
                                    <div key={inv.id} className="flex justify-between items-center p-4 border border-slate-100 rounded-xl hover:border-primary cursor-pointer transition-colors" onClick={() => setSelectedInvoice(inv)}>
                                        <div>
                                            ...
                                            <div className="text-xs text-slate-500">{fmtDate(inv.createdAt)} GÇó {inv.items.length} items</div>
                                        </div>
                                        <div className="font-bold text-lg text-emerald-600">{inv.totalAmount} +».+ú</div>
                                    </div>
                                ))}
                            </div>
                        )}
        ...
        <div dir="ltr" className="relative rounded-[2rem] md:rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(8,145,178,0.2)] bg-[#0a0f16] border border-sky-900/50 h-[220px] md:h-[280px] group select-none flex items-center">
             ...
             <div className="relative z-10 w-full px-8 md:px-16 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-0">
                 ...
                 <div className="flex flex-col items-center md:items-start relative">
                     ...
                     <div className="text-sky-500 text-xs font-mono tracking-[0.3em] mb-2 flex items-center gap-2">
                         <span className="w-2 h-2 bg-sky-400 rounded-full animate-ping"></span>
                         SYS.TIME.SYNC
                     </div>
                     
                     <div className="flex items-baseline gap-2 text-transparent bg-clip-text bg-gradient-to-b from-white via-sky-100 to-sky-500 drop-shadow-[0_0_20px_rgba(34,211,238,0.4)]">
                         ...
                         <span className="text-5xl md:text-7xl font-light text-sky-400 animate-pulse leading-none mb-4 md:mb-8">:</span>
                 ...
                 <div className="hidden md:flex flex-col items-end text-right z-20">
                     {/* Status Badge */}
                     <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-none border border-sky-500/30 bg-sky-950/40 backdrop-blur-sm mb-6 relative overflow-hidden group">
                        <div className="absolute inset-0 bg-gradient-to-r from-sky-500/10 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                        <i className="fa-solid fa-satellite-dish text-sky-400 animate-pulse"></i>
                        <span className="text-sky-300 font-mono text-xs uppercase tracking-[0.2em]">{t('system_name')} // ACTIVE</span>
                     </div>
                      ...
                      <div className="space-y-6">
                         <div className="bg-white p-5 md:p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                            ...
                            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
                               ...
                               <strong>+à+ä+º+¡+++¬:</strong> +¦+é+à +º+ä+ç+º+¬+ü +¦+è+â+ê+å +º+¦+à +º+ä+à+¦+¬+«+»+à+î +ê+â+ä+à+¬ +º+ä+à+¦+ê+¦ +¦+¬+Å+ê+ä+æ+» +¬+ä+é+º+ª+è+º+ï
                            </div>
                         </div>
                         <div className="bg-white p-5 md:p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <div className="text-[10px] text-slate-400 italic truncate font-medium">
                                                {app.isClass ? (
                                                    <span>{new Date(app.date).toLocaleTimeString('en-GB', {hour:'2-digit', minute:'2-digit'})} GÇó {app.reason}</span>
                                                ) : app.reason}
                                            </div>
                                        </div>
                                    </div>
