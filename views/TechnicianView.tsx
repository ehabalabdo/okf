...
const hearingLevels = [
  { value: 'normal', label: 'Ø·Ø¨ÙŠØ¹ÙŠ', color: 'bg-green-100 text-green-700' },
  { value: 'mild', label: 'Ø®ÙÙŠÙ', color: 'bg-yellow-100 text-yellow-700' },
  { value: 'moderate', label: 'Ù…ØªÙˆØ³Ø·', color: 'bg-orange-100 text-orange-700' },
  { value: 'moderately_severe', label: 'Ù…ØªÙˆØ³Ø· Ù„Ø´Ø¯ÙŠØ¯', color: 'bg-red-100 text-red-700' },
  { value: 'severe', label: 'Ø´Ø¯ÙŠØ¯', color: 'bg-red-200 text-red-800' },
  { value: 'profound', label: 'Ø¹Ù…ÙŠÙ‚', color: 'bg-red-300 text-red-900' },
];
...
const TechnicianView: React.FC = () => {
  ...
  const handleSubmitAudiogram = async () => {
    if (!selectedPatient) return alert('ÙŠØ±Ø¬Ù‰ Ø§Ø®ØªÙŠØ§Ø± Ø§Ù„Ù…Ø±ÙŠØ¶');
    setSaving(true);
    try {
    ...
    } catch (err: any) { alert(err.message || 'Ø­Ø¯Ø« Ø®Ø·Ø£'); }
    finally { setSaving(false); }
  };

  const handleSubmitBalance = async () => {
    if (!selectedPatient) return alert('ÙŠØ±Ø¬Ù‰ Ø§Ø®ØªÙŠØ§Ø± Ø§Ù„Ù…Ø±ÙŠØ¶');
    if (!balanceForm.diagnosis) return alert('ÙŠØ±Ø¬Ù‰ Ø¥Ø¯Ø®Ø§Ù„ Ø§Ù„ØªØ´Ø®ÙŠØµ');
    setSaving(true);
    try {
  ...
  const associatedLabels: Record<string, string> = {
    nausea: 'ØºØ«ÙŠØ§Ù†', vomiting: 'ØªÙ‚ÙŠØ¤', hearingLoss: 'Ø¶Ø¹Ù Ø³Ù…Ø¹',
    tinnitus: 'Ø·Ù†ÙŠÙ†', headache: 'ØµØ¯Ø§Ø¹', visualDisturbance: 'Ø§Ø¶Ø·Ø±Ø§Ø¨ Ø¨ØµØ±ÙŠ', fallHistory: 'ØªØ§Ø±ÙŠØ® Ø³Ù‚ÙˆØ·',
  };
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-slate-100 dark:bg-slate-700">
            <th className="p-2 text-right">Ø§Ù„ØªØ±Ø¯Ø¯ (Hz)</th>
            {frequencies.map(f => <th key={f} className="p-2 text-center">{freqLabels[f]}</th>)}
          </tr>
        </thead>
        <tbody>
          <tr className="border-b dark:border-slate-600">
            <td className="p-2 font-medium text-red-600">ÙŠÙ…Ù†Ù‰ <span className="text-xs">(O)</span></td>
            {frequencies.map(f => (
          ...
          <tr>
            <td className="p-2 font-medium text-amber-600">ÙŠØ³Ø±Ù‰ <span className="text-xs">(X)</span></td>
            {frequencies.map(f => (
              <td key={f} className="p-1">
  ...
  // ==================== RENDER ====================
  return (
    <Layout title="ÙÙ†ÙŠ Ø§Ù„ÙØ­ÙˆØµØ§Øª" hideTitle>
            ...
            <div className="h-full flex flex-col items-center justify-center p-10 text-center">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="text-right">
                  <h1 className="text-xl font-bold text-slate-800 dark:text-white">ÙÙ†ÙŠ Ø§Ù„ÙØ­ÙˆØµØ§Øª</h1>
              ...
              <div className="w-full max-w-2xl mt-8">
                <div className="flex items-center gap-3 mb-4 justify-center">
                  ...
                  <div className="text-right"><h2 className="font-bold text-slate-800 dark:text-white leading-tight">Ù†Ù…Ø§Ø°Ø¬ Ø§Ù„Ø£Ù†Ù ÙˆØ§Ù„Ø£Ø°Ù† ÙˆØ§Ù„Ø­Ù†Ø¬Ø±Ø©</h2><p className="text-[10px] text-slate-400 uppercase tracking-wide">ENT Medical Forms</p></div>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[
                    { path: 'ent/new-patient', icon: 'fa-file-medical', label: 'Ø§Ø³ØªØ¨ÙŠØ§Ù† Ù…Ø±ÙŠØ¶ Ø¬Ø¯ÙŠØ¯', color: 'bg-amber-50 text-amber-600 border-amber-200 hover:bg-amber-100' },
                    { path: 'ent/follow-up', icon: 'fa-file-lines', label: 'Ù…ØªØ§Ø¨Ø¹Ø© Ù…Ø±ÙŠØ¶', color: 'bg-green-50 text-green-600 border-green-200 hover:bg-green-100' },
                    { path: 'ent/audiogram', icon: 'fa-ear-listen', label: 'ÙØ­Øµ Ø§Ù„Ø³Ù…Ø¹', color: 'bg-purple-50 text-purple-600 border-purple-200 hover:bg-purple-100' },
                    { path: 'ent/balance', icon: 'fa-person-walking', label: 'ÙØ­Øµ Ø§Ù„ØªÙˆØ§Ø²Ù†', color: 'bg-amber-50 text-amber-600 border-amber-200 hover:bg-amber-100' },
                    { path: 'ent/referral', icon: 'fa-share-from-square', label: 'ØªØ­ÙˆÙŠÙ„ Ø·Ø¨ÙŠ', color: 'bg-rose-50 text-rose-600 border-rose-200 hover:bg-rose-100' },
                  ].map(item => {
