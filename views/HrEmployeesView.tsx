  ...
  return (
    <Layout title={isAr ? 'إدارة الموظفين - HR' : 'HR Employees'}>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <button onClick={openAdd} className="bg-amber-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-amber-700 shadow-lg transition-all flex items-center gap-2">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg my-8">
            <div className="bg-amber-600 p-5 rounded-t-2xl flex justify-between items-center text-white">
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                <input type="text" className="w-full p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none" value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} />
              </div>
              ...
              {!editingId && (
                <>
                  <div>
                    ...
                    <input type="text" className="w-full p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none font-mono" value={form.username} onChange={e => setForm({ ...form, username: e.target.value.toLowerCase().replace(/\s/g, '') })} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{isAr ? 'كلمة المرور' : 'Password'} *</label>
                    <div className="flex gap-2">
                      <input type="text" className="flex-1 p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none font-mono" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" className="w-full p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{isAr ? 'الإيميل' : 'Email'}</label>
                  <input type="email" className="w-full p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" step="0.01" min="0" className="w-full p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none" value={form.basic_salary} onChange={e => setForm({ ...form, basic_salary: parseFloat(e.target.value) || 0 })} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{isAr ? 'الصلاحية' : 'Role'}</label>
                  <select className="w-full p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none" value={form.role} onChange={e => setForm({ ...form, role: e.target.value as any })}>
              ...
              <div className="border-t border-slate-100 pt-4 mt-4">
                <h4 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                  <i className="fa-solid fa-clock text-amber-500"></i> {isAr ? 'جدول العمل' : 'Work Schedule'}
                </h4>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {DAYS.map(d => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${form.work_days.includes(d.value) ? 'bg-amber-600 text-white shadow' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                    >
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    ...
                    <input type="time" className="w-full p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none" value={form.start_time} onChange={e => setForm({ ...form, start_time: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{isAr ? 'نهاية الدوام' : 'End Time'}</label>
                    <input type="time" className="w-full p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none" value={form.end_time} onChange={e => setForm({ ...form, end_time: e.target.value })} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" min={0} max={60} className="w-full p-3 rounded-xl border border-gray-200 focus:border-amber-400 outline-none" value={form.grace_minutes} onChange={e => setForm({ ...form, grace_minutes: parseInt(e.target.value) || 0 })} />
                  </div>
                  <div className="flex items-end pb-1">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={form.overtime_enabled} onChange={e => setForm({ ...form, overtime_enabled: e.target.checked })} className="w-5 h-5 rounded accent-amber-600" />
            ...
            <div className="p-5 border-t border-slate-100 flex gap-3">
              ...
              <button onClick={handleSave} disabled={saving} className="flex-1 py-3 rounded-xl font-bold bg-amber-600 text-white shadow-lg hover:bg-amber-700 transition-colors disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md">
            ...
            <button
              ...
              className="w-full mb-4 py-2.5 rounded-xl border-2 border-dashed border-amber-200 text-amber-600 font-bold text-sm hover:bg-amber-50 transition-colors flex items-center justify-center gap-2"
            >
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSaveLocation} className="flex-1 py-3 rounded-xl font-bold bg-amber-600 text-white shadow-lg hover:bg-amber-700">
                {isAr ? 'حفظ' : 'Save'}
              </button>
            </div>
          </div>
