...
const CatalogView: React.FC = () => {
  ...
  const handleDeleteMed = async (id: string) => {
    if (!confirm(t('delete_medication_confirm'))) return;
    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (missingHeaders.length > 0) {
          alert(`${t('missing_columns')}: ${missingHeaders.join(', ')}`);
          return;
        }
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      alert(t('import_failed') + ': ' + e.message);
    }
    setImporting(false);
  };
          ...
          <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <div className="flex gap-2 flex-wrap">
              <button onClick={downloadServiceTemplate} className="px-3 py-2 bg-slate-100 text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-200 transition-all">
                <i className="fa-solid fa-download mr-1"></i> {t('download_template')}
              </button>
              ...
              <button onClick={() => serviceFileRef.current?.click()} className="px-3 py-2 bg-amber-100 text-amber-700 rounded-lg text-xs font-bold hover:bg-amber-200 transition-all">
                <i className="fa-solid fa-upload mr-1"></i> {t('upload_excel')}
              </button>
              <button onClick={() => setShowAddService(true)} className="px-3 py-2 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 transition-all">
                <i className="fa-solid fa-plus mr-1"></i> {t('add_service')}
              </button>
            </div>
          </div>
          ...
          {showAddService && (
            <div className="p-4 bg-emerald-50 border-b border-emerald-100">
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
                <input className="p-2 rounded-lg border text-sm" placeholder={t('service_name_required')} value={newService.serviceName} onChange={e => setNewService({ ...newService, serviceName: e.target.value })} />
                ...
                <input type="number" className="p-2 rounded-lg border text-sm" placeholder={t('price_required')} value={newService.price} onChange={e => setNewService({ ...newService, price: e.target.value })} />
                ...
                <div className="flex gap-2">
                  <button onClick={handleAddService} className="flex-1 bg-emerald-600 text-white rounded-lg font-bold text-sm hover:bg-emerald-700">{t('save')}</button>
                  <button onClick={() => setShowAddService(false)} className="px-3 bg-white border rounded-lg text-sm text-slate-500 hover:bg-slate-50">{t('cancel')}</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-xs uppercase text-slate-500">
                <tr>
                  <th className="text-left p-3 font-bold">{t('service_name_col')}</th>
                  <th className="text-left p-3 font-bold">{t('category_col')}</th>
                  <th className="text-right p-3 font-bold">{t('price_col')}</th>
                  <th className="text-center p-3 font-bold">{t('currency_col')}</th>
                  <th className="text-center p-3 font-bold">{t('status_col')}</th>
                  <th className="text-center p-3 font-bold">{t('actions_col')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {loadingServices ? (
                  <tr><td colSpan={6} className="text-center p-8 text-slate-400"><i className="fa-solid fa-spinner animate-spin mr-2"></i>{t('loading')}</td></tr>
                ) : services.length === 0 ? (
                  <tr><td colSpan={6} className="text-center p-8 text-slate-400">{t('no_services_hint')}</td></tr>
                ) : (
                  services.map(svc => (
                    <tr key={svc.id} className={`hover:bg-slate-50 transition-colors ${!svc.active ? 'opacity-50' : ''}`}>
                      ...
                      <td className="p-3 text-center">
                        <button onClick={() => toggleServiceActive(svc)} className={`px-2 py-0.5 rounded-full text-xs font-bold ${svc.active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                          {svc.active ? t('active_status') : t('inactive_status')}
                        </button>
                      </td>
                      <td className="p-3 text-center">
        ...
        <div className="bg-white rounded-2xl shadow-soft border border-slate-100 overflow-hidden">
          ...
          <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button onClick={() => setShowAddMed(true)} className="px-3 py-2 bg-amber-600 text-white rounded-lg text-xs font-bold hover:bg-amber-700 transition-all">
                <i className="fa-solid fa-plus mr-1"></i> {t('add_medication')}
              </button>
            </div>
          </div>
          ...
          {showAddMed && (
            <div className="p-4 bg-amber-50 border-b border-amber-100">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                <input className="p-2 rounded-lg border text-sm" placeholder={t('brand_name')} value={newMed.brandName} onChange={e => setNewMed({ ...newMed, brandName: e.target.value })} />
                <input className="p-2 rounded-lg border text-sm" placeholder={t('generic_name')} value={newMed.genericName} onChange={e => setNewMed({ ...newMed, genericName: e.target.value })} />
                <input className="p-2 rounded-lg border text-sm" placeholder={t('strength_placeholder')} value={newMed.strength} onChange={e => setNewMed({ ...newMed, strength: e.target.value })} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-3">
                <select className="p-2 rounded-lg border text-sm bg-white" value={newMed.dosageForm} onChange={e => setNewMed({ ...newMed, dosageForm: e.target.value })}>
                  <option value="">{t('dosage_form')}</option><option>Tablet</option><option>Capsule</option><option>Syrup</option><option>Injection</option><option>Cream</option><option>Drops</option><option>Inhaler</option><option>Suppository</option>
                </select>
                <select className="p-2 rounded-lg border text-sm bg-white" value={newMed.route} onChange={e => setNewMed({ ...newMed, route: e.target.value })}>
                  <option value="">{t('route_col')}</option><option>Oral</option><option>IV</option><option>IM</option><option>SC</option><option>Topical</option><option>Rectal</option><option>Inhalation</option><option>Ophthalmic</option><option>Otic</option>
                </select>
                <input className="p-2 rounded-lg border text-sm" placeholder={t('default_dose')} value={newMed.defaultDose} onChange={e => setNewMed({ ...newMed, defaultDose: e.target.value })} />
                <input className="p-2 rounded-lg border text-sm" placeholder={t('default_frequency')} value={newMed.defaultFrequency} onChange={e => setNewMed({ ...newMed, defaultFrequency: e.target.value })} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <input className="p-2 rounded-lg border text-sm" placeholder={t('default_duration')} value={newMed.defaultDuration} onChange={e => setNewMed({ ...newMed, defaultDuration: e.target.value })} />
                <input className="p-2 rounded-lg border text-sm" placeholder={t('notes_col')} value={newMed.notes} onChange={e => setNewMed({ ...newMed, notes: e.target.value })} />
                <div className="flex gap-2">
                  <button onClick={handleAddMed} className="flex-1 bg-amber-600 text-white rounded-lg font-bold text-sm hover:bg-amber-700">{t('save')}</button>
                  <button onClick={() => setShowAddMed(false)} className="px-3 bg-white border rounded-lg text-sm text-slate-500 hover:bg-slate-50">{t('cancel')}</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-xs uppercase text-slate-500">
                <tr>
                  <th className="text-left p-3 font-bold">{t('brand_name')}</th>
                  <th className="text-left p-3 font-bold">{t('generic_name')}</th>
                  <th className="text-left p-3 font-bold">{t('strength_col')}</th>
                  <th className="text-left p-3 font-bold">{t('form_col')}</th>
                  <th className="text-left p-3 font-bold">{t('route_col')}</th>
                  <th className="text-left p-3 font-bold">{t('default_dose')}</th>
                  <th className="text-center p-3 font-bold">{t('status_col')}</th>
                  <th className="text-center p-3 font-bold">{t('actions_col')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {loadingMeds ? (
                  <tr><td colSpan={8} className="text-center p-8 text-slate-400"><i className="fa-solid fa-spinner animate-spin mr-2"></i>{t('loading')}</td></tr>
                ) : medications.length === 0 ? (
                  <tr><td colSpan={8} className="text-center p-8 text-slate-400">{t('no_medications_hint')}</td></tr>
                ) : (
                  medications.map(med => (
                    <tr key={med.id} className={`hover:bg-slate-50 transition-colors ${!med.active ? 'opacity-50' : ''}`}>
                      ...
                      <td className="p-3 text-center">
                        <button onClick={() => toggleMedActive(med)} className={`px-2 py-0.5 rounded-full text-xs font-bold ${med.active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                          {med.active ? t('active_status') : t('inactive_status')}
                        </button>
                      </td>
                      <td className="p-3 text-center">
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5 border-b border-slate-100 flex justify-between items-center">
              <h3 className="text-lg font-extrabold text-slate-800">
                ...
                {t('import_preview')} — {importModal.rows.length} {t('rows_label')}
              </h3>
              ...
              <div className="p-4 bg-slate-50 border-b border-slate-100">
                <div className="flex gap-4 flex-wrap">
                  <div className="flex items-center gap-2 bg-green-100 text-green-700 px-3 py-1.5 rounded-lg font-bold text-sm">
                    <i className="fa-solid fa-circle-plus"></i> {t('created_count')}: {importResult.created}
                  </div>
                  <div className="flex items-center gap-2 bg-amber-100 text-amber-700 px-3 py-1.5 rounded-lg font-bold text-sm">
                    <i className="fa-solid fa-pen"></i> {t('updated_count')}: {importResult.updated}
                  </div>
                  {importResult.failed > 0 && (
                    <div className="flex items-center gap-2 bg-red-100 text-red-700 px-3 py-1.5 rounded-lg font-bold text-sm">
                      <i className="fa-solid fa-circle-xmark"></i> {t('failed_count')}: {importResult.failed}
                    </div>
                  )}
                </div>
            ...
            <div className="flex-1 overflow-auto p-4">
              ...
              {importModal.rows.length > 50 && (
                <p className="text-xs text-slate-400 mt-2 text-center">{t('showing_first_50')} {importModal.rows.length} {t('rows_label')}</p>
              )}
            </div>
            ...
            <div className="p-4 border-t border-slate-100 flex justify-end gap-3">
              <button onClick={() => { setImportModal(null); setImportResult(null); }} className="px-4 py-2 bg-slate-100 text-slate-600 rounded-lg font-bold text-sm hover:bg-slate-200">
                {importResult ? t('close_label') : t('cancel')}
              </button>
              {!importResult && (
                <button onClick={handleConfirmImport} disabled={importing} className="px-6 py-2 bg-emerald-600 text-white rounded-lg font-bold text-sm hover:bg-emerald-700 disabled:opacity-50 transition-all">
                  {importing ? (
                    <><i className="fa-solid fa-spinner animate-spin mr-2"></i>{t('importing_text')}</>
                  ) : (
                    <><i className="fa-solid fa-file-import mr-2"></i>{t('confirm_import')} ({importModal.rows.length} {t('rows_label')})</>
                  )}
                </button>
              )}
            </div>
