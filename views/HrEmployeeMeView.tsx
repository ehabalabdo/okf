        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-soft border border-gray-100 p-6">
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-500 to-purple-500 flex items-center justify-center text-white font-extrabold text-xl shadow-lg shadow-amber-200">
            ...
            {sched && (
              <div className="mt-5 pt-5 border-t border-slate-100">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {[1, 2, 3, 4, 5, 6, 7].map(d => (
                    <span key={d} className={`px-3 py-1.5 rounded-xl text-xs font-bold ${sched.workDays.includes(d) ? 'bg-amber-100 text-amber-700' : 'bg-slate-50 text-slate-300'}`}>
                      {isAr ? DAY_LABELS_AR[d] : DAY_LABELS_EN[d]}
                    </span>
                  ))}
                </div>
                <div className="flex gap-6 text-sm">
                  <span className="text-slate-500"><i className="fa-solid fa-clock text-amber-400 me-1"></i> {sched.startTime} — {sched.endTime}</span>
                  ...
                  {sched.overtimeEnabled && <span className="text-amber-600 font-bold"><i className="fa-solid fa-bolt me-1"></i> OT</span>}
                </div>
              </div>
            )}
          </div>
          ...
          {!profile.bioRegistered && (
            <div className="bg-gradient-to-br from-amber-50 to-purple-50 rounded-2xl border-2 border-dashed border-amber-300 p-6 text-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button
                ...
                className={`p-6 rounded-2xl font-extrabold text-lg transition-all flex flex-col items-center gap-2 ${
                  !onBreak
                    ? 'bg-slate-100 text-slate-400 cursor-not-allowed'
                    : 'bg-gradient-to-br from-amber-500 to-sky-600 text-white hover:from-amber-600 hover:to-sky-700 shadow-lg shadow-amber-200 active:scale-95 animate-pulse'
                }`}
              >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-soft border border-gray-100 p-5">
            <h3 className="font-extrabold text-sm text-slate-500 uppercase mb-4">
              <i className="fa-solid fa-calendar-day me-1 text-amber-400"></i>
            ...
            {today ? (
              <div className="space-y-3">
                ...
                {today.overtimeMinutes > 0 && (
                  <div className="flex items-center justify-between">
                    <span className="text-amber-500 text-sm">{isAr ? 'إضافي' : 'Overtime'}</span>
                    <span className="font-bold text-amber-600">{today.overtimeMinutes}m</span>
                  </div>
                )}
                <div className="pt-2 text-center">
                  <span className={`px-3 py-1.5 rounded-full text-xs font-bold ${
                    ...
                    today.status === 'incomplete' ? 'bg-amber-100 text-amber-700' :
          ...
          <div className="bg-white rounded-2xl shadow-soft border border-gray-100 p-5">
            <h3 className="font-extrabold text-sm text-slate-500 uppercase mb-4">
              <i className="fa-solid fa-chart-pie me-1 text-amber-400"></i>
            ...
            {summary ? (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-amber-600">{fmtMinutes(summary.totalOvertimeMinutes)}</span>
                </div>
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 space-y-2 text-xs text-slate-500">
            ...
            <p><i className="fa-solid fa-fingerprint text-amber-400 me-1"></i> {isAr ? 'البصمة مطلوبة للتحقق من هويتك' : 'Biometric required to verify your identity'}</p>
